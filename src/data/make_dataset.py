        ...
        if self.train:
            for file_idx in range(8):
                if file_idx < 5 and f"train_{file_idx}.npz" not in files:
                    ...
                    shutil.move(f"train_{file_idx}.npz",
...
def main(input_filepath: str, output_filepath: str) -> None:
    ...
    train = CorruptMnist(train=True, in_folder=input_filepath,
                         out_folder=output_filepath)
    train.save_preprocessed()

    test = CorruptMnist(train=False, in_folder=input_filepath,
                        out_folder=output_filepath)
